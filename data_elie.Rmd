...
```{r}
list.of.packages <- c("quantmod", "tidyquant","pylr","dplyr","redav","tidyverse", "remotes","naniar", "rollRegres", "zoo")
...
portfolio_close <- log(rowSums(close_prices))
spy_close <- log(spy_close)
beta_comparison <- data.frame(portfolio_close, spy_close)
beta_regression<-roll_regres(portfolio_close ~ spy_close, beta_comparison, width = 30,
